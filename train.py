...
if args.debug:
    ...
    os.system(
        "python  ./tools/train.py configs/{architecture}/{configPath} --seed 0 --work_dir models/{experiment}/{dataset}_{architecture}_{model}_detection".format(
            architecture=architecture,
    ...
    os.system(
        "python -m torch.distributed.launch --nproc_per_node={numDevices} ./tools/train.py configs/{architecture}/{configPath} --work_dir models/{experiment}/{dataset}_{architecture}_{model}_detection".format(
            architecture=architecture,
            configPath=configPath,
            experiment=experiment,
            model=model,
