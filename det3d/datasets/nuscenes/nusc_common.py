...
def _second_det_to_nusc_box(detection):
    ...
    for i in range(box3d.shape[0]):
        ...
        velocity0 = (*box3d[i, 6:8], 0.0)
        acceleration =(*box3d[i, 8:10], 0.0)
        box = Box(
            center=box3d[i, :3],
            size=box3d[i, 3:6],
            orientation=quat,
...
def get_annotations(nusc, annotations, ref_boxes, timesteps, past):
    ...
    for annotation, ref_box in zip(annotations, ref_boxes):
        ...
        for i in range(timesteps):
            velocity0, acceleration = nusc.box_velocity_Acceleration(annotation["token"])
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, nsweeps=20, filter_zero=True, timesteps=7,
    ...
    for sample in tqdm(nusc.sample):
        ...
        if not test:
            ...
            velocity0 = [np.array([b.velocity0 for b in boxes]).reshape(-1, 3) for boxes in forecast_boxes]
            acceleration = [np.array([b.acceleration for b in boxes]).reshape(-1, 3) for boxes in forecast_boxes]
            rots = [np.array([quaternion_yaw(b.orientation) for b in boxes]).reshape(-1, 1) for boxes in forecast_boxes]
            rrots = [np.array([quaternion_yaw(b.orientation) for b in boxes]).reshape(-1, 1) for boxes in
                     forecast_boxes]
            ...
            gt_boxes = [np.concatenate([locs[i], dims[i], velocity[i][:, :2], rvelocity[i][:, :2], velocity0[i][:, :2], acceleration[i][:, :2],
                                        -rots[i] - np.pi / 2,
                                        -rrots[i] - np.pi / 2], axis=1) for i in range(len(annotations))]
            # gt_boxes = np.concatenate([locs, dims, rots], axis=1)
            assert len(annotations) == len(gt_boxes) == len(velocity) == len(rvelocity) == len(velocity0) == len(acceleration)
            ...
            if len(annotations) > 0:
                if not filter_zero:
                    ...
                    info["gt_boxes_velocity0"] = np.array(velocity0)
                    info["gt_boxes_acceleration"] = np.array(acceleration)
                    ...
                    info["gt_boxes_velocity0"] = np.array(velocity0)[mask, :]
                    info["gt_boxes_acceleration"] = np.array(acceleration)[mask, :]
                ...
                velocity0 = np.array([b.velocity0 for b in ref_boxes]).reshape(-1, 3)
                acceleration = np.array([b.acceleration for b in ref_boxes]).reshape(-1, 3)
                rots = np.array([quaternion_yaw(b.orientation) for b in ref_boxes]).reshape(-1, 1)
                names = np.array([b.name for b in ref_boxes])
                tokens = np.array([b.token for b in ref_boxes])
                gt_boxes = np.concatenate([locs, dims, velocity[:, :2], rvelocity[:, :2], velocity0[:, :2],acceleration[:, :2],-rots - np.pi / 2], axis=1)
                trajectory = np.array(["static" for b in ref_boxes])

                info["gt_boxes"] = gt_boxes
                info["gt_boxes_velocity"] = velocity
                info["gt_boxes_rvelocity"] = rvelocity
                info["gt_boxes_velocity0"] = velocity0
                info["gt_boxes_acceleration"] = acceleration
...
def create_nuscenes_infos(root_path, version="v1.0-trainval", experiment="trainval_forecast", nsweeps=20,
                          filter_zero=True, timesteps=7, past=False):
    ...
    if test:
        ...
        with open(
                root_path / "{}/infos_train_{:02d}sweeps_withvelo_filter_{}.pkl".format(experiment, nsweeps,
                                                                                        filter_zero), "wb"
        ) as f:
            pickle.dump(train_nusc_infos, f)
        with open(
                root_path / "{}/infos_val_{:02d}sweeps_withvelo_filter_{}.pkl".format(experiment, nsweeps, filter_zero),
                "wb"
        ) as f:
