...
class NuScenes:
    ...
    def __init__(self,
                 ...
                 dataroot: str = '/media/asghar/nuscen_mini_data/NUSCENES_DATASET_ROO',
                 verbose: bool = True,
...
def box_past_center_acele(time, boxes):
    shift = [(box.center[:2] - (time * box.velocity0[:2]) - (
            0.5 * time * time * box.acceleration[:2])).tolist() for box in boxes]
    center_box = np.array(shift)
    return center_box


def box_future_center_acele(time, boxes):
    shift = [(box.center[:2] + (time * box.velocity0[:2]) + (
            0.5 * time * time * box.acceleration[:2])).tolist() for box in boxes]
    center_box = np.array(shift)
    return center_box
    ...
    if classname in ["car", "pedestrian"]:
        ...
        for timesteps, tm in zip(window(ret_boxes, 2), time):
           ...
           # curr_future = box_future_center(tm, current)
            curr_future = box_future_center_acele(tm, current)
        ...
        for idx in np.arange(len(ret_boxes[0])):
            ...
            velocity0 = curr.velocity0
            aceleration = curr.acceleration

            forecast = [curr]
            for t in time:
                new_box = deepcopy(forecast[-1])
             #   new_box.center = new_box.center + t * velocity
                new_box.center = new_box.center + (t * velocity0) +( 0.5 * aceleration *t*t)
        ...
        for timesteps, tm in zip(window(reverse_ret_boxes, 2), reverse_time):
            ...
            curr_past = box_past_center_acele(tm, current)
...
def box_serialize(box, token, name, attr):
    ret = {"sample_token": token,
           ...
           "velocity0": box.velocity0[:2].tolist(),
           "acceleration": box.acceleration[:2].tolist(),
           "detection_name": name,
...
def modify_forecast_boxes(elapse_time, det_forecast, forecast, forecast_mode, classname, jitter, K, C):
    ret_boxes, ret_tokens = [], []
        ...
        if not eval_only:
            for j, det_forecast in enumerate(tqdm(dets)):
                ...
                for i, boxes in enumerate(det_boxes):
                    ...
                    if np.sqrt(box.velocity0[0] ** 2 + box.velocity0[1] ** 2) > 0.2:
                        if name in [
                            "car",
