...
from nuscenes.eval.common.loaders import (add_center_dist, filter_eval_boxes,
                                          load_gt, load_prediction,load_predictionFutureDet)
...
def box2d_iou(boxA, boxB):
    ...
    polyA = Polygon(
        [(cornersA[0][0], cornersA[0][1]), (cornersA[1][0], cornersA[1][1]), (cornersA[5][0], cornersA[5][1]),
         (cornersA[4][0], cornersA[4][1])])
    polyB = Polygon(
        [(cornersB[0][0], cornersB[0][1]), (cornersB[1][0], cornersB[1][1]), (cornersB[5][0], cornersB[5][1]),
...
def trajectory_DAT(nusc, box: DetectionBox) -> float:
    ...
    vel = linear_forecast['velocity0'][:2]
...
def trajectory_FutureDet(nusc, box: DetectionBox) -> float:
    target = box.forecast_boxes[-1]
    time = [get_time(nusc, token[0], token[1]) for token in window([b["sample_token"] for b in box.forecast_boxes], 2)]
...
parser.add_argument('--model', default="forecast_n3dtfm")
...
parser.add_argument('--outputDirectory', default="Comparison_visualize/")
...
det_dir_FutureDet = "/media/asghar/media/FutureDet - Acceleration_2023_09_11/models/FutureDet/nusc_centerpoint_forecast_n3dtf_detection/infos_val_20sweeps_withvelo_filter_True.json"
...
if not os.path.isdir("{outputDirectory}/{experiment}/{dataset}_{architecture}_{model}_detection".format(
        outputDirectory=outputDirectory, experiment=experiment, dataset=dataset, architecture=architecture,
        model=model)):
    os.makedirs("{outputDirectory}/{experiment}/{dataset}_{architecture}_{model}_detection".format(
        outputDirectory=outputDirectory, experiment=experiment, dataset=dataset, architecture=architecture,
        model=model), exist_ok=True)
...
pred_boxes_FutureDet, meta_FutureDet = load_predictionFutureDet(det_dir_FutureDet, cfg.max_boxes_per_sample, DetectionBox, verbose=True)
...
for sample_token in tqdm(gt_boxes.boxes.keys()):
    ...
    pred_FutureDet = pred_boxes_FutureDet.boxes[sample_token]
    pred_confs_FutureDet = [box.forecast_score for box in pred_FutureDet]
    sortind_FutureDet = [i for (v, i) in sorted((v, i) for (i, v) in enumerate(pred_confs_FutureDet))][::-1]
    color_FutureDet = len(pred_FutureDet) * [("r", "r", "r")]
    ...
    taken_FutureDet = set()  # Initially no gt bounding box is matched.
    fid_FutureDet = set()
    for ind1 in sortind_FutureDet:
        pred_box_FutureDet = pred_FutureDet[ind1].forecast_boxes[0]

        if pred_FutureDet[ind1].forecast_id in fid_FutureDet:
            color_FutureDet[ind1] = ("r", "r", "r")
            continue
        ...
        for gt_idx, gt_box in enumerate(gt):
            ...
            # Find closest match among ground truth boxes
            if not gt_idx in taken_FutureDet:
                this_distance1 = center_distance(gt_box.forecast_boxes[0], pred_box_FutureDet)

                if this_distance1 < min_dist:
                    min_dist = this_distance1
                    match_gt_idx = gt_idx
        ...
        if is_match:
            taken_FutureDet.add(match_gt_idx)
            fid_FutureDet.add(pred_FutureDet[ind1].forecast_id)
            color_FutureDet[ind1] = ("m", "m", "m")

    pred_traj = []
    pred_traj_FutureDet = []
    for box in pred:
        label = trajectory_DAT(nusc, box)
        pred_traj.append(label)

    for box in pred_FutureDet:
        #label = trajectory_FutureDet(nusc, box)
        label  = "linear"
        pred_traj_FutureDet.append(label)

    gt_traj = []
    for box in gt:
        label = trajectory_DAT(nusc, box)
        gt_traj.append(label)
    pred.extend(pred_FutureDet)
    pred_traj.extend(pred_traj_FutureDet)
    color.extend(color_FutureDet)

    visualize_sample_forecast(nusc, sample_token, gt, pred, gt_traj, pred_traj, color, savepath="{}".format(
