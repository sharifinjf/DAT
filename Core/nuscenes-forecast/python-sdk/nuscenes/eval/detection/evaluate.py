...
def trajectory(nusc, box: DetectionBox) -> float:
    ...
    vel0 = linear_forecast.velocity0[:2]
    acele = linear_forecast.acceleration[:2]

    #disp = np.sum(list(map(lambda x: np.array(list(vel) + [0]) * x, time)), axis=0)
    disp = np.sum(list(map(lambda x: np.array(list(vel0) + [0]) * x, time)), axis=0)
    #disp = np.sum(list(map(lambda x: np.array(list(vel0) + [0]) * x + 0.5 *np.array(list(vel0) + [acele]) * x*x, time)), axis=0)
