...
class EvalBox(abc.ABC):
    ...
    def __init__(self,
                 ...
                 velocity0: Tuple[float, float] = (0, 0),
        ...
        assert len(velocity0) == 2, 'Error: Velocity0 ust have 2 elements!'
        # acceleration can be NaN from our database for certain annotations.
        assert len(acceleration) == 2, 'Error: acceleration ust have 2 elements!'
...
class EvalBoxes:
    ...
    @classmethod
    def deserializeFuturDet(cls, content: dict, box_cls):
        """
        Initialize from serialized content.
        :param content: A dictionary with the serialized content of the box.
        :param box_cls: The class of the boxes, DetectionBox or TrackingBox.
        ...
        eb = cls()
        for sample_token, boxes in content.items():
            eb.add_boxes(sample_token, [box_cls.deserializeFutreDet(box) for box in boxes])
        return eb
