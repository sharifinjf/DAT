...
def forecast_annotation(nusc, sample_annotation, sample_token, name, attr, timesteps=7):
    ...
    for i in range(timesteps):
        velocity0, acceleration = nusc.box_velocity_Acceleration(sample_annotation["token"])
        ...
        forecast_box.append({'sample_token' : sample_annotation["sample_token"],
                                    ...
                                    'velocity0': box.velocity0[:2],
                                    'acceleration':box.acceleration[:2],
                                    'detection_name' : name,
...
def load_predictionFutureDet(result_path: str, max_boxes_per_sample: int, box_cls, verbose: bool = False) \
        -> Tuple[EvalBoxes, Dict]:
    """
    Loads object predictions from file.
    :param result_path: Path to the .json result file provided by the user.
    ...
    # Deserialize results and get meta data.
    all_results = EvalBoxes.deserializeFuturDet(data['results'], box_cls)
    meta = data['meta']
    if verbose:
        print("Loaded results from {}. Found detections for {} samples."
              .format(result_path, len(all_results.sample_tokens)))
        ...
        for sample_annotation_token in sample_annotation_tokens:
            ...
            if box_cls == DetectionBox:
                ...
                if forecast_boxes is None:
                    continue
                velocity0, accelaration = nusc.box_velocity_Acceleration(sample_annotation["token"])
                sample_boxes.append(
                    box_cls(
                        ...
                        velocity0=velocity0[:2],
                        acceleration=accelaration[:2],
...
def filter_eval_boxes(nusc: NuScenes,
    ...
    for ind, sample_token in enumerate(eval_boxes.sample_tokens):
        ...
        bikerack_boxes = [Box(center=rec['translation'], size=rec['size'], orientation=Quaternion(rec['rotation'])) for rec in bikerack_recs]
        filtered_boxes = []
        for box in eval_boxes[sample_token]:
