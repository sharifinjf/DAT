...
class NuScenes:
    ...
    def get_box(self, sample_annotation_token: str) -> Box:
        ...
        velocity0, acceleration = self.box_velocity_Acceleration(record['token'])
        return Box(center=record['translation'], size=record['size'], orientation=Quaternion(record['rotation']),
                   name=record['category_name'], token=record['token'], velocity=velocity, velocity0=velocity0,
        ...
        if curr_sample_record['prev'] == "" or sd_record['is_key_frame']:
            ...
            for curr_ann_rec in curr_ann_recs:
                ...
                if curr_ann_rec['instance_token'] in prev_inst_map:
                    ...
                    velocity0, acceleration = self.box_velocity_Acceleration(curr_ann_rec['token'])
                    box = Box(center=center, size=curr_ann_rec['size'], orientation=rotation,
                              name=curr_ann_rec['category_name'],
                              token=curr_ann_rec['token'], velocity=velocity, velocity0=velocity0,
                              acceleration=acceleration)
                else:
    ...
    def box_velocity_Acceleration_0(self, sample_annotation_token: str, max_time_diff: float = 1.5) -> np.ndarray:
        """
        Estimate the velocity for an annotation.
        If possible, we compute the centered difference between the previous and next frame.
        Otherwise we use the difference between the current and previous/next frame.
        ...
        if has_prev:
            ...
            time_first = 1e-6 * self.get('sample', first['sample_token'])['timestamp']
        second = current
        pos_second = np.array(second['translation'])
        time_second = 1e-6 * self.get('sample', second['sample_token'])['timestamp']

        if has_next:
            third = self.get('sample_annotation', current['next'])
            pos_third = np.array(third['translation'])
            time_third = 1e-6 * self.get('sample', third['sample_token'])['timestamp']
        # calculate velocity when one frame is missed
        if not has_next and has_prev:
            ...
            if time_diff > max_time_diff:
                ...
                return pos_diff / time_diff, np.array([0, 0, 0])

        if has_next and not has_prev:
            time_diff = (time_third - time_second)
            pos_diff = (pos_third - pos_second)
            if time_diff > max_time_diff:
                # If time_diff is too big, don't return an estimate.
                #  return np.array([np.nan, np.nan, np.nan]), np.array([np.nan, np.nan, np.nan])
                return np.array([0, 0, 0]), np.array([0, 0, 0])
                ...
                return pos_diff / time_diff, np.array([0, 0, 0])
        # calculate velocity and acceleration when there are three points

        if has_next and has_prev:
            Time_Diff_Third_First = time_third - time_first
            Time_Diff_Third_Second = time_third - time_second
            Time_Diff_Second_First = time_second - time_first
            Pose_Diff_Third_First = pos_third - pos_first
            Pose_Diff_Third_Second = pos_third - pos_second
            Pose_Diff_Second_First = pos_second - pos_first

            if Time_Diff_Third_First > max_time_diff * 2 or \
                    Time_Diff_Third_Second > max_time_diff or Time_Diff_Second_First > max_time_diff:
                if Time_Diff_Second_First < max_time_diff:
                    return Pose_Diff_Second_First / Time_Diff_Second_First, np.array([0, 0, 0])
                elif Time_Diff_Third_Second < max_time_diff:
                    return Pose_Diff_Third_Second / Time_Diff_Third_Second, np.array([0, 0, 0])
                elif Time_Diff_Third_First < max_time_diff * 2:
                    return Time_Diff_Third_First / Pose_Diff_Third_First, np.array([0, 0, 0])
                else:
                    # return np.array([np.nan, np.nan, np.nan]), np.array([np.nan, np.nan, np.nan])
                    return np.array([0, 0, 0]), np.array([0, 0, 0])
            else:
                if (Time_Diff_Second_First != 0 and Pose_Diff_Third_Second != 0).all():
                    Accelerat = (2.00 * Pose_Diff_Third_First / (Time_Diff_Third_Second * Time_Diff_Third_First)) \
                                - (2.00 * Pose_Diff_Second_First / (Time_Diff_Second_First * Pose_Diff_Third_Second))
                    Velocity = Pose_Diff_Second_First / Time_Diff_Second_First - 0.5 * Accelerat * Time_Diff_Second_First
                    return Velocity, Accelerat
                else:
                    # return np.array([np.nan, np.nan, np.nan]), np.array([np.nan, np.nan, np.nan])
                    return np.array([0, 0, 0]), np.array([0, 0, 0])

    def box_velocity_Acceleration(self, sample_annotation_token: str, max_time_diff: float = 1.5,
                                  windowLength: float = 15, AceleTreshold: float = 0.3) -> np.ndarray:
        """
        Estimate the velocity for an annotation.
        If possible, we compute the centered difference between the previous and next frame.
        Otherwise we use the difference between the current and previous/next frame.
        ...
        current = self.get('sample_annotation', sample_annotation_token)
        current0 = self.get('sample_annotation', sample_annotation_token)
        has_prev = current['prev'] != ''
        has_next = current['next'] != ''

        pos0 = np.array(current['translation'])
        time0 = 1e-6 * self.get('sample', current['sample_token'])['timestamp']
        ...
        for n in range(windowLength - 1):
            if has_next:
                current = self.get('sample_annotation', current['next'])
                pos = np.array(current['translation'])
                time = 1e-6 * self.get('sample', current['sample_token'])['timestamp']
                has_next = current['next'] != ''
                Times.append(time)
                Poss.append(pos)
            elif has_prev:
                current0 = self.get('sample_annotation', current0['prev'])
                pos = np.array(current0['translation'])
                time = 1e-6 * self.get('sample', current0['sample_token'])['timestamp']
                has_prev = current0['prev'] != ''
                Times.append(time)
                Poss.append(pos)
        Poss = Poss - pos0
        Times = np.array(Times) - time0
        if Poss.shape[0] == 2 or Poss.shape[0] == 3:
            if Times[1] > 0:
                return np.array(Poss[1, :] / Times[1]), np.array([0, 0, 0])
            else:
                return np.array([0, 0, 0]), np.array([0, 0, 0])
        else:
            Ax, Vx, x0 = np.polyfit(Times, Poss[:, 0], 2, full=False)  # Coefficients for a, v0, and x0
            if abs(Ax) < AceleTreshold / 2:
                Ax = 0
                Vx, x0 = np.polyfit(Times, Poss[:, 0], 1, full=False)
                #if abs(Vx) < AceleTreshold:
                 #   Vx =0

            Ay, Vy, x0 = np.polyfit(Times, Poss[:, 1], 2, full=False)  # Coefficients for a, v0, and x0
            if abs(Ay) < AceleTreshold / 2:
                Ay = 0
                Vy, x0 = np.polyfit(Times, Poss[:, 1], 1, full=False)
              #  if abs(Vy) < AceleTreshold:
               #     Vy =0

            Az, Vz, x0 = np.polyfit(Times, Poss[:, 2], 2, full=False)  # Coefficients for a, v0, and x
            if abs(Az) < AceleTreshold / 2:
                Az = 0
                Vz, x0 = np.polyfit(Times, Poss[:, 2], 1, full=False)
               # if abs(Vz) < AceleTreshold:
                #    Vz =0

            return np.array([Vx, Vy, Vz]), np.array([2 * Ax, 2 * Ay, 2 * Az])
