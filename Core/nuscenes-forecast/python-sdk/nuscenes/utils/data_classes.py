...
class Box:
    ...
    def __init__(self,
                 ...
                 velocity0: Tuple = (np.nan, np.nan, np.nan),
                 acceleration: Tuple = (np.nan, np.nan, np.nan),
                 name: str = None,
                 token: str = None):
        """
        ...
        self.velocity0 = np.array(velocity0)
    ...
    def __eq__(self, other):
        ...
        vel0 = (np.allclose(self.velocity0, other.velocity0) or
               (np.all(np.isnan(self.velocity0)) and np.all(np.isnan(other.velocity0))))

        acele = (np.allclose(self.acceleration, other.acceleration) or
               (np.all(np.isnan(self.acceleration)) and np.all(np.isnan(other.acceleration))))
        return center and wlh and orientation and label and score and vel and acele and vel0

    def __repr__(self):
        repr_str = 'label: {}, score: {:.2f}, xyz: [{:.2f}, {:.2f}, {:.2f}], wlh: [{:.2f}, {:.2f}, {:.2f}], ' \
                   'rot axis: [{:.2f}, {:.2f}, {:.2f}], ang(degrees): {:.2f}, ang(rad): {:.2f}, ' \
                   'vel: {:.2f}, {:.2f}, {:.2f}, vel0: {:.2f}, {:.2f}, {:.2f}, acele: {:.2f}, {:.2f}, {:.2f}, name: {}, token: {}'
        ...
        return repr_str.format(self.label, self.score, self.center[0], self.center[1], self.center[2], self.wlh[0],
                               ...
                               self.velocity0[0], self.velocity0[1], self.velocity0[2],
    ...
    def rotate(self, quaternion: Quaternion) -> None:
        ...
        self.velocity0 = np.dot(quaternion.rotation_matrix, self.velocity0)
        self.acceleration = np.dot(quaternion.rotation_matrix, self.acceleration)
